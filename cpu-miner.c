 ...
 * Copyright 2014 The Pebblecoin Developers
 *
...
static enum algos opt_algo = ALGO_BOULDERHASH;
...
static threadpool_t *boulderhash_thread_pool = NULL;
...
Options:\n\
  ...
  -t, --threads=N       number of miner threads (default: 1)\n\
...
static inline void work_copy(struct work *dest, const struct work *src) {
    ...
    if (opt_algo == ALGO_BOULDERHASH)
    {
        memcpy(dest->result_hash, src->result_hash, sizeof(src->result_hash));
    }
}
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
    ...
    if (have_stratum) {
        ...
        if (jsonrpc_2) {
            ...
            char *hashhex = bin2hex((unsigned char*)work->result_hash, 32);
...
static void *miner_thread(void *userdata) {
    ...
    if (opt_algo == ALGO_BOULDERHASH) {
        ctx = create_boulderhash_ctx(boulderhash_thread_pool);
        
        if (!ctx)
        {
            applog(LOG_ERR, "boulderhash context creation failed");
            exit(1);
        }
    }
    ...
    while (1) {
        ...
        switch (opt_algo) {
        case ALGO_BOULDERHASH:
            rc = scanhash_boulderhash(thr_id, work.data, work.target,
                                      max_nonce, &hashes_done, ctx, work.result_hash);
            break;
...
int main(int argc, char *argv[]) {
	...
	if (!opt_benchmark && !opt_testing && !rpc_url) {
	...
	if (num_processors < 1)
		num_processors = 1;
	if (!opt_n_threads)
		opt_n_threads = (opt_algo == ALGO_BOULDERHASH ? 1 : num_processors - 1);
    ...
    if (opt_algo == ALGO_BOULDERHASH)
    {
        boulderhash_thread_pool = threadpool_create(num_processors, opt_n_threads * 65 * 2, 0);
        if (!boulderhash_thread_pool)
        {
            applog(LOG_ERR, "boulderhash threadpool create failed");
            return 1;
        }
    }
    ...
    if (opt_testing)
    {
        if (opt_algo != ALGO_BOULDERHASH)
        {
            applog(LOG_ERR, "testing only enabled for boulderhash");
            return 1;
        }
        ...
        if (!pc_boulderhash(ctx, data, 5, (char *)hash))
        {
            applog(LOG_ERR, "failed to boulderhash");
            return 1;
        }
        
        printf("Hash for 'hello' is: ");
        for (int i=0; i < 32; i++) {
